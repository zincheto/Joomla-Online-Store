...
<?php foreach ($products as $product) : ?>
<li><div class="pricefr"><?php echo JHTML::link(JRoute::_('index.php?option=com_virtuemart&view=productdetails&virtuemart_product_id='.$product->virtuemart_product_id.'&virtuemart_category_id='.$product->virtuemart_category_id), $product->product_name, array('title' => $product->product_name,'rel'=>'facebox')); ?><?php
	if ($show_price) {
		echo $currency->priceDisplay($product->prices['salesPrice']);
		if ($product->prices['salesPriceWithDiscount']>0) echo $currency->priceDisplay($product->prices['salesPriceWithDiscount']);
	}?></div> 
